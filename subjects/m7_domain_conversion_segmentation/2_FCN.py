...
from custom_utils import get_device, \
    ...
    model_save_in_safetensors, make_model_dir, get_optional_args, \
    load_statedict_in_safetensors
...
if __name__ == "__main__":
    ...
    if sys_options['inference_stage']:
        infered_path = os.path.join(results_path, '2_fcn_inference')
        os.makedirs(infered_path, exist_ok=True)
        model_safetensors_path = os.path.join('model', '2_FCN.safetensors')
        state_dict = load_statedict_in_safetensors(model_safetensors_path)
        model = UNet(n_channels=3, n_classes=3).to(device)
        model.load_state_dict(state_dict)

        for name, param in model.named_parameters():
            assert torch.equal(param, state_dict[name]), f"Parameter {name} not loaded correctly!"
        ...
        for i in range(n_test_points):
            ...
            display_prediction(model, img, gt, device, infered_path)
